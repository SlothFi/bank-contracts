...
describe('MasterBreeder::Authorization', () => {
  ...
  beforeEach(async () => {
    ...
    // 1000 SLOTH per block, rewards start at block 0, rewards are halved after every 45360 blocks
    breeder = await deployMasterBreeder(wallets, govToken, REWARDS_PER_BLOCK, REWARDS_START_BLOCK, HALVING_AFTER_BLOCK_COUNT)
  })

  it("should allow the owner to reclaim ownership of the Sloth token", async function () {
  ...
  it("should allow authorized users to reclaim ownership of the Sloth token", async function () {
