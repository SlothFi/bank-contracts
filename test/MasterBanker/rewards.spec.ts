...
import { deployMasterBanker, deployGovernanceToken, TOKEN_NAME, TOKEN_SYMBOL, TOTAL_CAP, MANUAL_MINT_LIMIT } from '../shared/deploy'
...
describe('MasterBanker::Rewards', () => {
  ...
  context("Entering & withdrawing from pools + claiming rewards", function () {
    ...
    it("should not pay out MON rewards before farming has started", async function () {
      ...
      await expect(banker.connect(bob).deposit(0, 0, ZERO_ADDRESS)).to.be.reverted

      await banker.connect(bob).deposit(0, expandTo18Decimals(100), ZERO_ADDRESS)

      expect(await govToken.totalSupply()).to.equal(0)
      
      await banker.connect(bob).claimReward(0)
      expect(await govToken.totalSupply()).to.equal(0)
      expect(await govToken.balanceOf(bob.address)).to.equal(expandTo18Decimals(0))
    })

    it("should pay out MON rewards after farming has started", async function () {
      this.timeout(0)
      ...
      const activeMultiplier = await banker.getMultiplier(currentBlock.number, currentBlock.number+1)
      const firstMultiplier = await banker.REWARD_MULTIPLIER(0)
      expect(activeMultiplier).to.equal(firstMultiplier)

      const rewardPerBlock = await banker.REWARD_PER_BLOCK()
      ...
      await expect(banker.connect(bob).claimReward(0))
        .to.emit(banker, 'SendGovernanceTokenReward') // emit SendGovernanceTokenReward(msg.sender, _pid, pending, lockAmount);
      ...
      const { forDev, forFarmer, forLP, forCom, forFounders } = await banker.getPoolReward(currentBlock.number, currentBlock.number+1, rewardsPerBlock)
      ...
      await banker.add(rewardsPerBlock*rewardsMultiplierForSecondPool, lp2.address, true) //5x bonus rewards pool vs pool 0
      await lp2.connect(carol).approve(banker.address, expandTo18Decimals(1000))
      await banker.connect(carol).deposit(1, expandTo18Decimals(100), ZERO_ADDRESS)

      // she should have two times (two sets of rewards since we're at block 102) 5x (=10x) of Bob's block 101 rewards
      await expect(banker.connect(carol).claimReward(1))
        .to.emit(banker, 'SendGovernanceTokenReward') // emit SendGovernanceTokenReward(msg.sender, _pid, pending, lockAmount);
      ...
      await lp2.connect(bob).approve(banker.address, expandTo18Decimals(1000))
      await banker.connect(bob).deposit(1, expandTo18Decimals(100), ZERO_ADDRESS)

      // Advance 10 blocks, then claim rewards from all pools
      advanceBlockWith(provider, 10)
      await banker.connect(bob).claimRewards([0, 1])

      expect('claimReward').to.be.calledOnContractWith(banker, [0]);
      expect('claimReward').to.be.calledOnContractWith(banker, [1]);
    ...
    it("should not distribute MONs if no one deposit", async function () {
      this.timeout(0)
      const debugMessages = false
      // 1 per block farming rate starting at block 600 with the first halvening block starting 1000 blocks after the start block
      const rewardsPerBlock = 1
      const banker = await deployMasterBanker(wallets, govToken, expandTo18Decimals(rewardsPerBlock), 600, 1000)
      await govToken.transferOwnership(banker.address)
      await banker.add(rewardsPerBlock, lp.address, true)
      await lp.connect(bob).approve(banker.address, expandTo18Decimals(1000))
      ...
      // block 610: 
      await expect(banker.connect(bob).deposit(0, expandTo18Decimals(10), ZERO_ADDRESS)) 
        .to.emit(banker, 'Deposit') //emit Deposit(msg.sender, _pid, _amount);
      ...
      await expect(banker.connect(bob).withdraw(0, expandTo18Decimals(10), ZERO_ADDRESS)).to.be.reverted

      // calculate the user's deposit
      const userDepositFee = await banker.userDepFee()
      ...
      await expect(banker.connect(bob).withdraw(0, likelyDeposit, ZERO_ADDRESS)) 
        .to.emit(banker, 'Withdraw') //emit Withdraw(msg.sender, _pid, _amount);
    ...
    it("should distribute MONs properly for each staker"), async () => {
      // 1 per block farming rate starting at block 300 with the first halvening block starting 1000 blocks after the start block
      const rewardsPerBlock = 1
      const banker = await deployMasterBanker(wallets, govToken, expandTo18Decimals(rewardsPerBlock), 300, 1000)

      await govToken.transferOwnership(banker.address)
      await banker.add(rewardsPerBlock, lp.address, true)
      await lp.connect(alice).approve(banker.address, expandTo18Decimals(1000))
      await lp.connect(bob).approve(banker.address, expandTo18Decimals(1000))
      await lp.connect(carol).approve(banker.address, expandTo18Decimals(1000))
      // Alice deposits 10 LPs at block 310
      await advanceBlockTo(provider, 309)
      await banker.connect(alice).deposit(0, expandTo18Decimals(10), ZERO_ADDRESS)
      // Bob deposits 20 LPs at block 314
      await advanceBlockTo(provider, 313)
      await banker.connect(bob).deposit(0, expandTo18Decimals(20), ZERO_ADDRESS)
      // Carol deposits 30 LPs at block 318
      await advanceBlockTo(provider, 317)
      await banker.connect(carol).deposit(0, expandTo18Decimals(30), ZERO_ADDRESS)
      ...
      expect(await govToken.balanceOf(banker.address)).to.equal(expandTo18Decimals(4334))
      ...
      await banker.connect(bob).withdraw(0, expandTo18Decimals(5), ZERO_ADDRESS)
      ...
      expect(await govToken.balanceOf(carol.address)).to.equal(0)
      expect(await govToken.balanceOf(banker.address)).to.equal(expandTo18Decimals(8144))
      ...
      await banker.connect(alice).withdraw(0, expandTo18Decimals(20), ZERO_ADDRESS)
      await advanceBlockTo(provider, 349)
      await banker.connect(bob).withdraw(0, expandTo18Decimals(15), ZERO_ADDRESS)
      await advanceBlockTo(provider, 359)
      await banker.connect(carol).withdraw(0, expandTo18Decimals(30), ZERO_ADDRESS)
    ...
    it("should give proper MONs allocation to each pool"), async () => {
      ...
      const rewardsPerBlock = 1
      const banker = await deployMasterBanker(wallets, govToken, expandTo18Decimals(rewardsPerBlock), 400, 1000)
      ...
      await banker.add(rewardsPerBlock*2, lp2.address, true) // 2x bonus
      // Alice should have 10*1000 pending reward
      expect(await banker.pendingReward(0, alice.address)).to.equal(expandTo18Decimals(10000))
      // Bob deposits 10 LP2s at block 425
      await advanceBlockTo(provider, 424)
      await banker.connect(bob).deposit(1, expandTo18Decimals(5), ZERO_ADDRESS)
      // Alice should have 10000 + 5*1/3*1000 = 11666 pending reward
      expect(await banker.pendingReward(0, alice.address)).to.equal(expandTo18Decimals(11666))
      await advanceBlockTo(provider, 430)
      // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
      expect(await banker.pendingReward(0, alice.address)).to.equal(expandTo18Decimals(13333))
      expect(await banker.pendingReward(1, bob.address)).to.equal(expandTo18Decimals(3333))
    }
  })
