...
// Referrals aren't actively used by the MasterBanker contract - no automatic payouts will be handed out to the referrer on deposits & withdrawals
...
describe('MasterBanker::Referrals', () => {
  ...
  beforeEach(async () => {
    ...
    banker = await deployMasterBanker(wallets, govToken, expandTo18Decimals(rewardsPerBlock), rewardsStartAtBlock, 1000)
  ...
  it("should properly track referrals", async function () {    
    ...
    await banker.connect(bob).deposit(0, expandTo18Decimals(100), alice.address)

    // The contract should now keep track of Alice's referral
    let refValue = await banker.getRefValueOf(alice.address, bob.address)
    let globalRefValue = await banker.getGlobalRefAmount(alice.address)
    expect(refValue).to.eq(expandTo18Decimals(100))
    expect(globalRefValue).to.eq(expandTo18Decimals(100))
    expect(await banker.getTotalRefs(alice.address)).to.eq(1)

    // Alice now also refers Carol
    await lp.connect(carol).approve(banker.address, expandTo18Decimals(1000))
    await banker.connect(carol).deposit(0, expandTo18Decimals(100), alice.address)
    
    refValue = await banker.getRefValueOf(alice.address, carol.address)
    globalRefValue = await banker.getGlobalRefAmount(alice.address)
    expect(refValue).to.eq(expandTo18Decimals(100))
    expect(globalRefValue).to.eq(expandTo18Decimals(200))
    expect(await banker.getTotalRefs(alice.address)).to.eq(2)

    // calculate the user's deposit
    let userDepositFee = await banker.userDepFee()
    let likelyDeposit = expandTo18Decimals(10).sub(expandTo18Decimals(10).mul(userDepositFee).div(10000))
    await banker.connect(bob).withdraw(0, likelyDeposit, alice.address)

    // Bob withdraws from the pool and Alice's referral value/score should be lowered as a consequence
    expect(await banker.getRefValueOf(alice.address, bob.address)).to.lt(refValue)
    expect(await banker.getGlobalRefAmount(alice.address)).to.lt(globalRefValue)
